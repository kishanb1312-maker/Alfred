...
# /warmapply-approve — approval-gate only
...
2. **Invoke** the `warmapply:approval-gate` subagent → for each job at "Ready for Review": send a
...
Honor **/pause**. Approving does not itself apply — the application-agent (`/warmapply-apply` or a full
