---
description: Run the full WarmApply pipeline (reconcile → find → research → tailor → track → approve → apply → report), preflight-gated. Honors dry_run, /pause, and daily caps.
disable-model-invocation: true
---

# /warmapply-run — the whole pipeline

Execute the WarmApply runbook end to end. **This changes no agent behavior** — it is the exact
sequence in `RUNBOOK.md`, only gated by a preflight check. Invoke each subagent in order and
**stop on any blocker**.

Run the bundled scripts with the WarmApply Python env — `${WARMAPPLY_HOME:-$HOME/.warmapply}/.venv/bin/python`
...
## The sequence
1. **Reconcile** — invoke the `warmapply:approval-gate` subagent to poll Telegram for taps made since
   the last run: apply Approvals (hand to `warmapply:application-agent`), record Skips. Clears the backlog first.
2. **Find** — invoke `warmapply:job-finder` → fresh, de-duped jobs (respects the configured sources).
3. **Research** — invoke `warmapply:company-research` per job → company analysis + verified email
   (waterfall) + match score. Drop jobs below `match_threshold`.
4. **Tailor** — invoke `warmapply:resume-cover-letter` per surviving job → tailored `.docx`/`.pdf`,
   cover letter, outreach email, and the mandatory `what_i_changed.md`. Merge into the canonical job
   object (see `RUNBOOK.md`).
5. **Track** — invoke `warmapply:tracker` → create/update a Notion row per job at Status "Ready for Review".
6. **Approve** — invoke `warmapply:approval-gate` → send a Telegram card per job; poll briefly for
   immediate taps. Un-tapped jobs wait for a later run.
7. **Apply** — invoke `warmapply:application-agent` → process `data/approved_queue.json`, **respecting
